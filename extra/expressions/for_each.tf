...
# Maps are a way to create variables that are lookup tables. An example will show this best. Example extracting AMIs into a map and add support for the us-west-2 region as well:
variable "amis" {
  type = "map"
  default = {
...
# Example used in the bare resource...
plan = "${var.plans["5USD"]}"
...
# lookup(map, key, default) retrieves the value of a single element from a map, given its key. If the given key does not exist, the given default value is returned instead
